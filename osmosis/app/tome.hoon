...
=<
  ...
  ++  on-save
    ^-  vase
    ~>  %bout.[0 '%osmosis +on-save']
    !>(state)
  ::
  ++  on-load
    |=  ole=vase
    ~>  %bout.[0 '%osmosis +on-load']
    ^-  (quip card _this)
    =^  cards  state
      abet:(load:eng ole)
    [cards this]
  ...
  ++  on-peek
    |=  =path
    ~>  %bout.[0 '%osmosis +on-peek']
    ^-  (unit (unit cage))
    (peek:eng path)
  ::
  ++  on-agent
    |=  [pol=(pole knot) sig=sign:agent:gall]
    ~>  %bout.[0 '%osmosis +on-agent']
    ^-  (quip card _this)
  ...
  ++  on-arvo
    ...
    ~>  %bout.[0 '%osmosis +on-arvo']
    ^-  (quip card _this)
    `this
  ::
  ++  on-watch
  |=  =path
  ~>  %bout.[0 '%osmosis +on-watch']
  ^-  (quip card _this)
  =^  cards  state  abet:(watch:eng path)
  [cards this]
  ::
  ++  on-fail
    ~>  %bout.[0 '%osmosis +on-fail']
    on-fail:def
  ::
  ++  on-leave
    ~>  %bout.[0 '%osmosis +on-leave']
    on-leave:def
  --
